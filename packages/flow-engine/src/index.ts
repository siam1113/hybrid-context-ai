...
export class FlowMemoryStore {
  ...
  findByIntent(applicationId: string, intent: string): FlowGraph | undefined {
    ...
    return [...this.flows.values()]
      ...
      .map((flow) => ({ flow, score: flow.intentExamples.reduce((score, example) => score + lexicalOverlap(normalized, example.toLowerCase()), 0) }))
      .sort((a, b) => b.score - a.score)[0]?.flow;
  }
  createPlan(applicationId: string, intent: string): ExecutionPlan {
    ...
    return { id: `plan_${Date.now()}`, intent, applicationId, flowGraphId: flow.id, nodes: topologicalSort(flow), edges: flow.edges, runtimeState };
  }
